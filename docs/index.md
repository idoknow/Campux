...
1. 第一次部署：先读 [部署与快速开始](/getting-started)。
2. 接手一个校园墙：读 [运营工作台](/operator/overview) 和 [审核与发布](/operator/review-and-publish)。
3. 接入机器人：读 [机器人管理](/operator/bots) 和 [OneBot 接入](/reference/onebot)。
4. 维护生产环境：读 [运维面板](/admin/overview)、[部署与升级](/admin/deployment) 和 [安全基线](/admin/security)。
