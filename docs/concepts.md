...
| 平台 | 运营管理员 | 可进入运营面板，创建校园墙，并管理自己作为租户管理员加入的校园墙 |
...
3. 如果账号是系统运维或运营管理员，会额外显示对应的运维/运营面板入口。
4. 如果访问域名绑定了某个校园墙 host，则账号会被锁定到该校园墙，不再展示租户选择。

运营管理员不是系统运维。运营管理员看到的是自己负责的校园墙集合；用户列表、审计日志、队列状态和租户生命周期都会按这些校园墙收窄。

账号有多个校园墙身份时，登录后进入的就是这个选择页；系统运维和运营管理员还会看到独立的“进入运维面板”入口：
