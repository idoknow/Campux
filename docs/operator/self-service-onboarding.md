...
这篇文档描述官方服务 [app.campux.top](https://app.campux.top) 预期的完整自助链路：墙号运营者访问管理端，注册运营管理员账号，创建自己的校园墙，接入 NapCat，然后开始收稿、审核和发布。
...
flowchart LR
  ...
  H --> I["扫码登录 QZone"]
  I --> J["测试投稿、审核、发布"]
  click A "https://app.campux.top" "打开 Campux 官方服务"
```

从 [app.campux.top](https://app.campux.top) 进入管理端后，按图中的顺序完成配置。

## 1. 打开管理端并注册

系统运维需要先在运维面板配置“管理端 host”，例如 [app.campux.top](https://app.campux.top)。
...
- 运维面板里的管理端 host 已设置为 [app.campux.top](https://app.campux.top)。
