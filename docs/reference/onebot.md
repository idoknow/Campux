...
```text
私聊投稿支持中文 `#` 和英文 `#`。`#投稿` 后面的内容会作为正文的起始部分，发送后会先进入待确认状态；你需要回复 `#匿名` 或 `#实名` 选择投稿方式，然后才会进入草稿状态。进入草稿后可以继续发送正文或图片；如果发送文字，Campux 会提示请发送 `#结束` 提交稿件。投稿过程中也可以发送 `#取消` 放弃本次投稿。
```
...
私聊投稿支持中文 `#` 和英文 `#`。`#投稿` 后面的内容会作为正文的起始部分，发送后会先进入待确认状态；你需要先回复 `#匿名` 或 `#实名` 选择投稿方式，然后才会进入草稿状态。进入草稿后可以继续发送正文或图片；如果发送文字，Campux 会提示请发送 `#结束` 提交稿件。投稿过程中也可以发送 `#取消` 放弃本次投稿。
