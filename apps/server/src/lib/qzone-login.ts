...
export async function pollQZoneQrLogin(taskId: string) {
  ...
  const leased = await runWithActiveTenantLease(prisma, task.tenantId, async (transaction) => {
    ...
    return { committed: true as const, cookieNames: Object.keys(cookies) };
  });
  if (!leased.active) {
    throw new Error("校园墙已暂停或归档");
  }
  if ("committed" in leased.value) {
    task.status = "succeeded";
    task.message = "登录完成";
    task.cookieNames = leased.value.cookieNames;
    return serializeTask(task);
  }
  return leased.value;
}
