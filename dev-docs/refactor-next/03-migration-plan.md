...
- 系统运维后台：独立于普通校园墙工作台，负责租户生命周期、全局用户、membership、域名绑定、系统队列和审计状态。
- 登录后的校园墙选择器：仅当账户被授权进入多个校园墙时展示；只有一个校园墙时直接进入；普通用户页面不展示租户切换。
- 租户域名/slug 映射。
- 租户内展示配置：校园墙名称、slug、主题色、前台品牌名、公告、投稿规则、服务入口由当前校园墙的 `admin` 维护。
- 租户内成员管理：`admin` 可管理自己校园墙的 `submitter`、`reviewer`、`admin` 授权；系统运维只在独立运维面板查看和处理全局授权/审计，不混入普通租户页面。
...
- 系统运维面板必须独立进入，不作为某个校园墙的 tab。
- 系统运维面板第一层只处理全局视角和生命周期，例如运行中、暂停、归档。
