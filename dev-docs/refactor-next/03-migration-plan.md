...
1. 登录和账号体系：把 QQ uin 迁成全局 `User`。一个用户只有一个账户，通过 `TenantMembership` 授权进入一个或多个校园墙；租户内角色收敛为 `submitter`、`reviewer`、`admin`，平台/系统身份作为用户账户级 `operations_admin` 或 `system_operator`。
...
- 系统/运营后台：独立于普通校园墙工作台；系统运维负责全局租户生命周期、全局用户、membership、域名绑定、系统队列和审计状态，运营管理员只能管理自己负责的校园墙集合。
- 登录后的校园墙选择器：仅当账户被授权进入多个校园墙时展示；只有一个校园墙时直接进入；普通用户页面不展示租户切换。
- 租户域名/slug 映射。
- 租户内展示配置：校园墙名称、slug、主题色、前台品牌名、公告、投稿规则、服务入口由当前校园墙的 `admin` 维护。
- 租户内成员管理：`admin` 可管理自己校园墙的 `submitter`、`reviewer`、`admin` 授权；运营管理员只能在自己负责的校园墙范围内处理成员；系统运维只在独立运维面板查看和处理全局授权/审计，不混入普通租户页面。
- 租户级 Bot 配置：多个墙号、审核群、帮助文本、发布延迟、QZone 登录。
- 租户级发布目标配置：同一篇投稿可同步发布到多个 QQ 墙号，并能单独启停、重试和查看失败原因。
- 租户级 S3 对象前缀和数据统计。
- 每个租户独立初始化，而不是全局 `/init` 只创建一个管理员。
- 租户级 UI 主题：品牌色、logo、前台展示文案和状态色；这些是租户内管理配置，不属于系统运维生命周期面板。

边界要求：

- 普通校园墙工作台只承载当前租户内的投稿、稿件、服务和管理能力。
- 系统/运营面板必须独立进入，不作为某个校园墙的 tab。
- 系统运维面板第一层处理全局视角和生命周期；运营管理员面板第一层只处理自己负责的校园墙，例如运行中、暂停、归档。
- 校园墙名称、slug、主题色、前台品牌名、公告等运营展示配置归属租户内 `admin`。
