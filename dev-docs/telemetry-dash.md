...
3. DNS：`dash.campux.top` → 服务器，建议走 Cloudflare 橙云代理：
   - 自动拿到 TLS；
   - 收集器读取 `CF-IPCountry`（国家）头用于国家维度。省份维度不依赖 CDN：收集器在入库时用上报来源 IP（`request.ip`，经 `trustProxy` 取 `CF-Connecting-IP`/`X-Forwarded-For`）做**离线** IP 归属查询（内置 ip2region xdb），直接解析出中国大陆省份名（如「广东省」）。
   - 因此无需在 Cloudflare 开启任何 Managed Transforms；province 维度即开即用、零外部请求。海外 / 内网 / 无法定位的 IP 解析为空，不计入「省份分布」。
